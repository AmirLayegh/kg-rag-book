...
movie_info_by_name_title_descroption = {
    "type": "function",
    "function": {
...
def movie_info_by_title(title: str):
 ...
 """
    records, _, _ = neo4j_driver.execute_query(cypher, title=title.lower())
    return [record.data() for record in records]
...
def movies_info_by_actor(actor: str):
    ...
    RETURN m AS movie, collect(a.name) AS cast, collect(d.name) AS directors
 """
    records, _, _ = neo4j_driver.execute_query(cypher, actor=actor.lower())
    return [record.data() for record in records]
